...
describe('Translator Component', () => {
    ...
    it('allows user to input text, select languages, and displays translations', async () => {
        // *jinx jinx* - Setup mock response
        mockedAxios.post.mockResolvedValueOnce({
            data: {
                translatedText: 'Translated (en): 测试'
            }
        });
        mockedAxios.post.mockResolvedValueOnce({
            data: {
                translatedText: 'Translated (ru): 测试'
            }
        });
        ...
        fireEvent.keyDown(languageSelector, { key: 'ArrowDown' }); // *jinx jinx* - Open dropdown
        ...
        expect(screen.getByText('en: Translated (en): 测试')).toBeInTheDocument();
        expect(screen.getByText('ru: Translated (ru): 测试')).toBeInTheDocument();
    });
